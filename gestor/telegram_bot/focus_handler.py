...
async def start_focus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_focus:
        # BUG 19 FIX: validar si la sesión realmente sigue activa comparando el tiempo
        if current_focus.get("end_time", 0) > time.time():
            await update.effective_message.reply_text("⚠️ Ya tienes una sesión de enfoque activa.")
            return
        else:
    ...
    await update.effective_message.reply_text(
        f"🚀 <b>Modo Enfoque activado</b> ({duration} min).\n"
...
async def finish_focus(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text="🔔 <b>¡Tiempo!</b> El bloque de enfoque ha terminado.\n\n"
...
async def stop_focus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("⏹ Modo enfoque cancelado.")
