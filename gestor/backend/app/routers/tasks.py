...
def update_task(
    ...
    if data.assignee_id is not None:
        # Track previous assignee to detect a real change
        assignment = db.query(TaskAssignment).filter(TaskAssignment.task_id == task.id).first()
        if assignment:
            previous_assignee_id = assignment.user_id
            assignment.user_id = data.assignee_id
        else:
            assignment = TaskAssignment(task_id=task.id, user_id=data.assignee_id, assigned_by=current_user.id)
            db.add(assignment)

        # Notify the new assignee proactively if they are different from the previous one
        if data.assignee_id != previous_assignee_id:
            from app.models.telegram import TelegramAccount
            from app.routers.telegram import send_telegram_message
            new_assignee_acc = db.query(TelegramAccount).filter(
                TelegramAccount.user_id == data.assignee_id,
                TelegramAccount.activo == True
            ).first()
            if new_assignee_acc:
                project_name = task.project.name if task.project else "Sin proyecto"
                send_telegram_message(
                    new_assignee_acc.telegram_chat_id,
                    f"📋 *Tarea reasignada*\n\n"
                    f"Se te ha asignado la tarea *{task.name}*\n"
                    f"Proyecto: {project_name}\n"
                    f"Reasignada por: {current_user.full_name}"
                )
