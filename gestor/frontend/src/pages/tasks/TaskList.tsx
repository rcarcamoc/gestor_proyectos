      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          <input 
            ...
            className="px-3 py-1.5 text-sm rounded-md bg-surface/50 border border-border/50 text-text-base placeholder-text-muted focus:outline-none focus:border-secondary"
          />
          <button
              ...
              {filteredTasks.map((task) => (
                <tr key={task.id} className="hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => { setCurrentTask(task); setIsDetailOpen(true); getSuggestions(task.id); }}>
                  ...
                  <td className="px-6 py-4 text-right">
                    ...
                    {task.status !== 'Completed' && (
                      <button onClick={(e) => { e.stopPropagation(); updateTaskStatus(task.id, 'Completed'); }} className="text-accent-green hover:underline text-xs" title="Marcar como completada">✓ Completar</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-background/80 backdrop-blur-md p-4">
          <div className="glass-card w-full max-w-2xl p-6 relative animate-in fade-in zoom-in duration-300 border border-border/50">
             ...
             <div className="flex gap-2 mb-6">
                 ...
                 <span className="px-2 py-1 bg-surface text-xs rounded border border-border/50">{STATUS_LABELS[currentTask.status] || currentTask.status}</span>
                 <span className="px-2 py-1 bg-surface text-xs rounded border border-border/50">{PRIORITY_LABELS[currentTask.priority] || currentTask.priority}</span>
             </div>
             ...
             <div className="grid grid-cols-2 gap-6 bg-surface/50 p-4 rounded-xl border border-border/50 mb-6">
                ...
                <div>
                   ...
                   {currentTask.status !== 'Completed' && (
                     <button onClick={() => { updateTaskStatus(currentTask.id, 'Completed'); setIsDetailOpen(false); }} className="w-full bg-accent-green text-white text-xs font-bold py-2 rounded-lg text-center shadow shadow-accent-green/20 mb-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <div>
                    <label className="block text-xs font-semibold text-text-muted uppercase mb-2 flex items-center justify-between">
                      Assignee
                      <button type="button" onClick={() => getSuggestions(formData.id || 0)} className="text-secondary hover:underline">Sugerir</button>
                    </label>
