...
async def start_anti_paralysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text(
        "💪 <b>Modo Anti-Parálisis activado</b>\n\n"
        "Te voy a acompañar paso a paso para que termines esa tarea que te está costando.\n\n"
        "Primero: ¿Tienes la herramienta, documento o aplicación abierta donde vas a trabajar?",
...
async def handle_open_app(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "Perfecto. Sigamos.\n\n"
        "¿Tienes claro cuál es el primer pequeño paso accionable que debes dar?",
        reply_markup=reply_markup
    )
    return STEP_SUBJECT
...
async def handle_subject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "Bien. Penúltimo paso:\n\n"
        "¿Tienes todo el material o información necesaria a la mano para dar ese paso?",
        reply_markup=reply_markup
    )
...
async def handle_recipient(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "🚀 <b>¡Todo listo!</b>\n\n"
        "Ya no hay más bloqueos. Empieza ahora mismo con ese primer paso.\n\n"
        "Avísame cuando hayas avanzado al menos 5 minutos o lo hayas terminado presionando el botón de abajo.",
        reply_markup=reply_markup,
        parse_mode='HTML'
    )
    return STEP_SEND
...
async def handle_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "¿Quieres que registremos este avance en SmartTrack?",
        parse_mode='HTML'
    )
    return ConversationHandler.END

async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_message.reply_text("Modo anti-parálisis desactivado. ¡Tú puedes!")
    return ConversationHandler.END
