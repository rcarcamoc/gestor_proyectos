            ...
            <div key={p.id} className="glass-card p-6 flex flex-col group relative overflow-hidden">
              ...
              <div className="flex justify-between items-start mb-4 relative z-10">
                ...
                <div className="flex gap-3">
                   <button onClick={() => window.location.href = `/tasks?project=${p.id}`} className="text-xs font-bold text-secondary hover:underline transition-colors">Ver Tareas</button>
                   <button onClick={() => { setEditingProject(p); setIsModalOpen(true); }} className="text-xs font-bold text-primary hover:underline transition-colors">Editar</button>
                </div>
              </div>
