...
# Si va después, FastAPI interpreta "active-timer" como un task_id int y lanza 422.
...
def start_timer(
    ...
    entry = TimeEntry(
        ...
        started_at=datetime.now(timezone.utc).replace(tzinfo=None),  # Guardamos naive UTC
        source="timer"
    )
    db.add(entry)
    db.commit()
...
def stop_timer(
    ...
    # Ambas son naive UTC, la resta es segura
