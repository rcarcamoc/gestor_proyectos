...
async def process_writing_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text("¿Qué tono prefieres?", reply_markup=reply_markup)
...
async def handle_tone_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(text=f"Generando borrador en tono <b>{tone}</b>...", parse_mode='HTML')

    client = get_http_client()
    res = await client.post("/telegram/completion", json={
        "system_prompt": f"Eres un asistente de redacción profesional. Ayuda al usuario a escribir un mensaje en tono {tone}. Devuelve el texto listo para copiar y pegar.",
        "user_prompt": user_input
    })
    
    if res.status_code == 200:
        response = res.json().get("response", "No se pudo generar el texto.")
    else:
        response = "Error al conectar con el servicio de IA central."
