...
class SmartEngine:
    ...
    def suggest_assignees(self, task_id: int, team_id: int) -> Dict[str, Any]:
        ...
        from app.models.task import TaskRequiredSkill

        members = self.db.query(TeamMembership).filter(TeamMembership.team_id == team_id).all()

        # Levels used to score how well a user's level meets the requirement
        LEVEL_RANK = {"basic": 1, "intermediate": 2, "advanced": 3, "expert": 4}

        # Retrieve required skills for this task (may be empty for tasks created before this feature)
        required_skills = self.db.query(TaskRequiredSkill).filter(TaskRequiredSkill.task_id == task_id).all()
        required_map = {rs.skill_id: rs.min_level for rs in required_skills}  # {skill_id: min_level}
        ...
        for member in members:
            ...
            # Find user full_name and skills
            ...
            user_skill_map = {us.skill_id: us.level for us in user_skills_db}  # {skill_id: level}

            # --- Real skill matching ---
            if required_map:
                # Calculate how many required skills are met at or above the required level
                met = 0
                for skill_id, min_level in required_map.items():
                    user_level = user_skill_map.get(skill_id)
                    if user_level and LEVEL_RANK.get(user_level, 0) >= LEVEL_RANK.get(min_level, 1):
                        met += 1
                match_percentage = round((met / len(required_map)) * 100)
                skills_note = f"{met}/{len(required_map)} skills requeridos cumplidos"
            else:
                # No required skills defined for this task — fall back to "has any skill" heuristic
                # but be transparent with a lower confidence cap
                has_skills = len(user_skill_map) > 0
                match_percentage = 60 if has_skills else 30
                skills_note = f"{len(user_skill_map)} skills registrados (tarea sin skills requeridos definidos)"

            score_total = (match_percentage * 0.7) + (availability_percentage * 0.3)
            justification = f"{skills_note}. Capacidad libre estimada: {capacity_left:.1f}h."
            ...
            candidates.append({
                ...
                "skills_count": len(user_skill_map),
                "required_skills_count": len(required_map),
                "match_percentage": match_percentage,
                "availability_percentage": round(availability_percentage, 1),
                "score_total": round(score_total, 1),
                "justification": justification,
                "is_recommended": not eval_result["has_conflicts"] and capacity_left > 0 and match_percentage >= 50
            })
