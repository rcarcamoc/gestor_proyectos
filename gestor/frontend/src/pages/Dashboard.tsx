...
const Dashboard: React.FC = () => {
  ...
  const toggleTaskComplete = async (taskId: number, currentStatus: string) => {
    ...
    setData((prev: any) => ({
      ...prev,
      tasks_today: prev.tasks_today.map((t: any) => 
              ...
              <div key={t.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between hover:border-blue-200 transition-colors">
                <div className="flex items-center space-x-4">
                  <input 
                    ...
                    onChange={() => toggleTaskComplete(t.id, t.status)}
                    className="w-5 h-5 rounded-lg border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer" 
                  />
                  <div>
                    <p className={`font-bold ${t.status === 'Completed' ? 'text-gray-400 line-through' : 'text-gray-800'}`}>{t.name}</p>
      ...
      <div className="w-full lg:w-72 mt-8 lg:mt-0 space-y-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          ...
          <ul className="space-y-2">
             <li><button disabled title="Próximamente" className="w-full text-left p-3 text-sm rounded-lg opacity-50 cursor-not-allowed">Registrar tiempo manual</button></li>
             <li><button disabled title="Próximamente" className="w-full text-left p-3 text-sm rounded-lg opacity-50 cursor-not-allowed">Solicitar ausencia</button></li>
             <li><button disabled title="Próximamente" className="w-full text-left p-3 text-sm rounded-lg font-bold opacity-50 cursor-not-allowed">Actualizar mis skills</button></li>
          </ul>
        </div>
      </div>
    </div>
