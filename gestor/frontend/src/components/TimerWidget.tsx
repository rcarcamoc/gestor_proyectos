...
const TimerWidget: React.FC<{ taskId: number, taskName: string }> = ({ taskId: initialTaskId, taskName: initialTaskName }) => {
  const { isActive, seconds, taskId, taskName, startTimer, stopTimer } = useTimer();

  const currentTaskId = taskId || initialTaskId;
  const currentTaskName = taskName || initialTaskName;
  ...
  const handleStart = () => startTimer(currentTaskId, currentTaskName);
  ...
  return (
    <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-4">
        ...
        {!isActive ? (
          <button onClick={handleStart} className="bg-green-500 hover:bg-green-600 text-text-base p-2 rounded-full shadow-lg transition-colors">
